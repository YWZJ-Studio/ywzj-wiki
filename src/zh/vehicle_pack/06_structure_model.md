...
## 分组与枢轴
与渲染模型类似，结构模型也需要良好分组，并且枢轴位置关系与渲染模型一致，才可保证在复杂旋转时OBB的逻辑方向与载具的视觉方向一致
...
**枢轴对齐**是本章稍显费力的环节，因为基岩模型中，枢轴是嵌套递归计算绝对位置的，所以渲染模型中某结构的枢轴坐标往往不能直接抄给结构模型
::: tip
...
## 特殊组
除`structure_bone`指定的部件组及武器约定组外，结构模型中还存在少量由游戏逻辑**硬编码**识别的特殊组名，它们不由载具配置引用，而是由引擎自动解析

### `vehicle_body`
**车体组**，是所有载具结构模型都必须包含的组，用于勾勒载具对实体的碰撞外形，若`main_structure`组不存在，其中最大的块则为载具物理块

### `main_structure`
**主结构组**，其中最大的块为载具物理块。物理块用于表达车体的物理效果，如重力作用下适应地形而产生的车体偏转、车体碰撞墙壁而停止等，若`aerodynamic_structure`组不存在，其朝向也是固定翼攻角为0时的朝向

### `aerodynamic_structure`
**气动结构组**，仅用于固定翼飞机，其中最大的块的朝向为攻角为零时的指向，常用于处理物理块与气动朝向存在夹角的情况
